...
const List = () => {
  ...
  const fetchData = async (page) => {
    ...
    if (networkStatus.connected) {
      const result = await http.get(process.env.REACT_APP_BASE_URL + '/atencion/' + page + '/' + state.rowsPerPage + '?' + new URLSearchParams(o).toString() + '&activo=0');
  ...
  const atenderOnClick = () => {
    dispatch({
      type: "confirm", msg: 'Se realizo la atención al registro seleccionado?', cb: (e) => {
        if (e) {
          http.put('/atencion/' + selected[0], { activo: 0 }).then(e => {
            navigate('/atencion');
    ...
    <>
      <Toolbar className="Toolbar-table mt-1" direction="row" >
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={3}>
            <Button sx={{ width: 250, fontWeight: 'bold' }} startIcon={<EditIcon />} variant="contained" color="error">Atendidos</Button>
          </Grid>
          <Grid item xs={12} md={3}>
